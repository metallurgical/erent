...
$sql_pelajar = "SELECT * FROM user where role = 'admin' or role = 'staff'" ;
...
if(($_GET['action']=="delete") && ($_GET['id']!=NULL))
{
	...
	if($result_padam_pelajar = $connect->query($sql_padam_pelajar))
	{
		if($total_padam_pelajar = $result_padam_pelajar->num_rows)
		{
			...
			if($result_padam_pelajar = $connect->query($sql_padam_pelajar))
			{
				echo "<script language=javascript>alert('Maklumat berjaya dipadam.');window.location='edit_delete_user.php';</script>";
			}
			else
			{
				echo "<script language=javascript>alert('Maklumat tidak berjaya dipadam. Sila cuba lagi.');window.location='edit_delete_user.php';</script>";
			}
		}
		else
		{
			echo "<script language=javascript>alert('Maklumat tidak wujud.');window.location='edit_delete_user.php';</script>";
		}
	}
}
?>
        ...
        <div class="span9">
		  <div class="row-fluid">
			<div class="page-header">
				<h1>Pengurusan Pengguna <small></small></h1>
			</div>
			<table class="table table-striped table-bordered table-condensed">
               ...
               <?php if($total_pelajar>0) { do { ?>   <tr>
                      ...
                      <a href="my-profile.php?id=<?php echo $rows_pelajar['user_id'];?>" title="kemaskini"> 
                     <i class="icon-pencil"></i> </a>
                          <a href="del_pentadbir.php?id=<?php echo $rows_pelajar['user_id'];?>" title="padam" onclick="return confirm('Anda Pasti Untuk Padam?')"> 
